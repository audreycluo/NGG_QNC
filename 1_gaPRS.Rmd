...
from jakob
- The HBID column will be the link between the data. 
- The “libi_” files are the PRS scores for each subject (use “Standardizes scores”) derived from PNC (has some overlap with CHOP kids)
- birthweight_CHOP file is the gestational age and birthweight numbers from the medical records from CHOP 
- the n278 file is the overlap with the imaging that was used for Ted’s paper. 
- PCs from the SNP array data (basically capturing genetic drift and ancestry effects). I am attaching the PCs and the basic demographics

- First step would be to look at the relationship between the respective PRS and phenotype. Feel free to ping me with any questions and I can of course send some more things as we go!


8/25:
•	Check association between PRS and gestational age and birthweight
o	May need to include PC’s in linear model
o	And I assume I’m doing separate linear models for GA and birthweight?
o	Can consider making GA a categorical variable (like extremely preterm <28 weeks, very preterm 28-32 weeks, and moderate-to-late preterm 32-27 weeks)
o	Outcome variable = gestational age or birthweight. Predictor variable = PRS
...
demographics
- how are race and ethnicity coded?
```{r}
head(dem)
length(intersect(dem$HBID, ga_bw_clean1$HBID))

# make sure dem subjects are in same order as ga_bw_clean1
dem_clean1 <- dem[match(ga_bw_clean1$HBID, dem$HBID),]
